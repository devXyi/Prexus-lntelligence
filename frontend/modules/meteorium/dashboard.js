...
import { fPct, fUsd, riskColor, riskLabel, riskClass } from '../../js/utils.js';
...
export async function init(container) {
  ...
  const alertCount = assets.filter(a => (a.cr||0) >= 0.65).length;
  ...
  container.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 280px;gap:12px;margin-bottom:12px">
      ...
      <div class="panel" style="display:flex;flex-direction:column;overflow:hidden">
        ...
        <div style="flex:1;overflow-y:auto">
          ${SIM_ASSETS.filter(a=>a.cr>=0.45).sort((a,b)=>b.cr-a.cr).slice(0,6).map(a=>_miniCard(a)).join('')}
        </div>
      </div>
    </div>
    ...
    <div style="margin-bottom:12px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        ${SIM_COMPOUNDS.map(c=>_compoundCard(c)).join('')}
      </div>
    </div>
...
function _miniCard(a) {
  ...
  return `<div style="padding:8px 12px;border-bottom:1px solid var(--border);border-left:2px solid ${c};background:${bg}">
    <div style="display:flex;align-items:center;gap:5px;margin-bottom:3px">
      ...
      <span style="font-size:9px;color:var(--text-primary)">${a.id}</span>
      <span style="margin-left:auto;font-family:var(--font-display);font-size:15px;color:${c}">${fPct(a.cr)}</span>
    </div>
    <div style="font-size:9px;color:var(--text-secondary);line-height:1.4">${a.name}</div>
...
function _compoundCard(ev) {
  ...
  const chainHTML=ev.chain.map((n,i)=>`
    <div class="met-cascade-node">
      ...
      <span class="met-cascade-label">${n.label}</span>
  ...
  return `<div style="background:${bgC};border:1px solid ${borderC};border-radius:3px;padding:12px 14px">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
      <span style="font-size:8px;font-weight:700;letter-spacing:.14em;padding:2px 6px;border-radius:2px;text-transform:uppercase;background:${badgeBg};color:${badgeTx};border:1px solid ${badgeBd}">${ev.sev}</span>
      <span style="font-size:9px;color:var(--text-secondary)">${ev.location}</span>
    </div>
    <div style="font-family:var(--font-display);font-size:14px;letter-spacing:.08em;text-transform:uppercase;color:${titleC};margin-bottom:4px">${ev.type.replace(/_/g,' ')}</div>
    <div style="font-family:var(--font-serif);font-size:11px;color:var(--text-secondary);line-height:1.55;font-style:italic;margin-bottom:10px">${ev.desc}</div>
...
async function _loadPortfolio() {
  ...
  try {
    ...
    if (r?.portfolio_composite_risk != null) p = { composite_risk:r.portfolio_composite_risk, var_95:r.portfolio_var_95, cvar_95:r.portfolio_cvar_95, loss_expected_mm:r.loss_expected_mm, total_value_mm:r.total_value_mm };
  } catch {}
  ...
  el.innerHTML = `<div style="display:grid;grid-template-columns:repeat(6,1fr);gap:14px">
    ${[['Total Exposure',fUsd(p.total_value_mm),'var(--cobalt)'],['Composite Risk',fPct(p.composite_risk),riskColor(p.composite_risk)],['VaR 95%',fPct(p.var_95),'var(--amber)'],['CVaR 95%',fPct(p.cvar_95),'var(--red)'],['Expected Loss',fUsd(p.loss_expected_mm),'var(--red)'],['Scenario','SSP2-4.5','var(--green)'],]
    .map(([l,v,c])=>`<div><div style="font-size:7.5px;color:var(--text-muted);letter-spacing:.1em;text-transform:uppercase;margin-bottom:3px">${l}</div><div style="font-family:var(--font-display);font-size:20px;color:${c};line-height:1">${v}</div></div>`).join('')}
  </div>`;
}

function _bindMapClicks(container) {
  container.addEventListener('click', e => {
    const dot = e.target.closest('.dash-asset-dot');
    if (!dot) return;
    const a = SIM_ASSETS.find(x => x.id === dot.dataset.asset);
    if (!a) return;
    ...
    el.innerHTML = `<div style="padding:10px 14px;border-top:1px solid var(--border);background:rgba(0,0,0,.3);display:flex;align-items:center;gap:16px">
      <div>
        <div style="font-family:var(--font-display);font-size:14px;color:white;letter-spacing:.08em">${a.name}</div>
        <div style="font-size:8.5px;color:var(--text-secondary)">${a.id} · ${a.lat.toFixed(2)}°, ${a.lon.toFixed(2)}°</div>
      </div>
      <div style="margin-left:auto;display:flex;gap:18px">
        ${[['Composite',a.cr],['Physical',a.pr],['Transition',a.tr]].map(([l,v])=>`<div><div style="font-size:7px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.1em">${l}</div><div style="font-family:var(--font-display);font-size:18px;color:${riskColor(v)}">${fPct(v)}</div></div>`).join('')}
      </div>
      <button onclick="document.getElementById('dash-detail').style.display='none'" style="background:none;border:none;color:var(--text-muted);cursor:pointer;font-size:14px;padding:0 4px">✕</button>
    </div>`;
  });
}
